...
- **Sign-in geometry (Observed, BRIEF-004):** SSO y174, OR y254, form top y324, fields 427×53 @ y324 (Email) / y412 (Password), CTA y500, footer y580 — same x=590 w=427 column; FA Sign-In (Sign Up 4) mirrors the same rhythm and has **no Full Name field**.
- **Layer dumps (Observed, BRIEF-004):** `docs/figma/active_dump.css` = Active look (authoritative for shipped pages); `docs/figma/dump.html` / `inactive_dump.*` = Inactive variant (underline fields 35px @ 309/374/439) — do not confuse when comparing.
- **Revised active-look geometry (2026-09-08, BRIEF-005/CLM-013 — user-confirmed rework target):** new `docs/figma/active/` exports (`Sign Up 1–4.svg` = EN/FA sign-up, EN/FA sign-in). New `docs/figma/inactive/` = the current shipped look (boxed fields SSO 164 / 294/372/450 / CTA 528) — matches built pages. **Observed (TASK-AUTH-03-01 implementation, measured from the SVGs):** the revised active look replaces the rounded-cap **box** chrome with a **single underline** per field row — EN sign-up SSO y**170** (187×44), icon rows y**316/381/446** (65px pitch), underline y**343.5/408.5/473.5**, CTA y**504**, footer y**580**, OR y**244**; sign-in SSO y**180**, icon rows y**346/421** (75px pitch), underline y**373.5/448.5**, CTA y**489**, footer y**564**, OR y**264**; FA mirrors the same rows (form x135). Field internals: icon at row top (EN left 5 / FA right 5), label beside icon (EN left 38 / FA right 38 pad), eye right 5 (EN) / left 5 (FA). Footer links are accent `#92CBCE` on all four pages (was muted on sign-up).
...
- Observed state specifics (state galleries + component sets; feeds AUTH-01-03 / AUTH-02-03):
  ...
  - Field internals (Observed, BRIEF-004): box top+bottom lines at L15 W403 (427 box, rounded caps); Big Label 16px / Small Label 12px = Source Serif Pro (floating label type).
